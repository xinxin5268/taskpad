...
description: 8 条 Agent 行为规则 + 资产保护 + 审计监管 + 主动关心。解决 Agent 闷头死循环、跑偏不自知、做完不汇报、泄露凭据、宕机全丢等痛点。
---
...
| 痛点 | 装之前 | 装之后 |
|------|--------|--------|
| 😤 **闷头死循环** | 同一个错连试 7 次还不停 | loop-detector 连续 3 次/2 次一致喊停 |
| 🏃 **跑偏不自知** | 加一个 --json 参数，顺手重构了五个模块 | drift-detector 连续 2 步偏离喊停 |
| 🤐 **做完不汇报** | 做完任务用户以为 Agent 死了 | reporter 完成汇报 + 超 2 分钟无动静主动刷存在 |
| 📛 **任务理解错误** | 用户说搜索框，Agent 做了前端 filter | confirmer 先输出理解等确认 |
| 💣 **泄露凭据** | 把 ~/.ssh/id_rsa 吐到对话里 | guardian 拦截 + 资产保护脱敏 |
| 🧠 **弱模型瞎猜** | 小模型把聊天当任务、把任务当聊天 | classifier 全关键词匹配分拣 |
| 💀 **宕机全丢** | Session 中断后 23 步任务蒸发 | checkpointer 每步写 checkpoint，resume 原地复活 |
| 🎉 **一句话干大事** | 一个大任务硬啃 3 小时结果乱七八糟 | task-breaker 拆原子步骤，一步步确认 |
| 💰 **财产操作无保护** | Agent 直接操作钱包/转账 | 资产保护 7 类 + 双重确认 + 自动脱敏 |
| 📝 **违规无追责** | Agent 违规用户不知道 | 5 层审计防线 + 熔断机制，Agent 改不了日志 |
| 🤷 **离线无回应** | 用户 1 小时不说话Agent 一动不动 | 主动关心 1 小时离线问候 |
